...
function sp_showPoll($parameters, $id, $return_parameters = false)
{
	...
	if ($poll['allow_vote'])
	{
		...
		echo '
										<li class="sp_center"><input type="submit" value="', $txt['poll_vote'], '" /></li>
...
function sp_rssFeed($parameters, $id, $return_parameters = false)
{
	...
	if ($only_titles)
	{
		...
		foreach ($items as $item)
			echo '
									<li>', sp_embed_image('dot_feed'), ' ', $item['link'], '</li>';
	...
	else
	{
		...
		foreach ($items as $item)
			echo '
										<li class="sp_list_top">', sp_embed_image('post'), ' <strong>', $item['link'], '</strong></li>
		...
		echo '
									<tr>
										<td class="sp_staff sp_center">', !empty($staff['avatar']['href']) ? '
											<img src="' . $staff['avatar']['href'] . '" alt="' . $staff['name'] . '" width="40" />' : '', '
										</td>
...
function sp_shoutbox($parameters, $id, $return_parameters = false)
{
	...
	$shoutbox = sportal_get_shoutbox($parameters['shoutbox'], true);
				...
				echo '
									<tr>
										<td class="sp_top_poster sp_blog">', !empty($blog['avatar']['href']) ? '
											<img src="' . $blog['avatar']['href'] . '" alt="' . $blog['name'] . '" width="40" />' : '', '
										</td>
										<td>
