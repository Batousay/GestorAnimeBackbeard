...
function sportal_init_headers()
{
	...
	<script type="text/javascript" src="' . $settings['default_theme_url'] . '/scripts/portal.js?233"></script>
...
function getBlockInfo($column_id = null, $block_id = null, $state = null, $show = null)
{
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		...
		if (!sp_allowed_to('block', $row['id_block'], $row['allowed_groups'], $row['permission_type']))
			continue;
...
function getShowInfo($block_id = null, $display = null, $custom = null)
{
	...
	{
		$page_info = sportal_get_pages($_GET['page'], true);
		if (empty($page_info))
			return false;
	}
...
function sp_allowed_to($type, $id, $perm_groups = null, $perm_type = null)
{
	...
	if (!isset($perm_groups) || !isset($perm_type))
	{
		$request = $smcFunc['db_query']('','
			SELECT {raw:groups}, {raw:type}
			FROM {db_prefix}sp_{raw:table}
			WHERE {raw:id} = {int:id_item}
			LIMIT {int:limit}',
			array(
				'groups' => $types[$type]['groups'],
		...
		list ($perm_groups, $perm_type) = $smcFunc['db_fetch_row']($request);
		$smcFunc['db_free_result']($request);
	}

	$result = false;
	$cache_name = $perm_type . '-' . $perm_groups;

	if (isset($cache[$cache_name]))
		$result = $cache[$cache_name];
	else
	{
		$groups = explode(',', $perm_groups);

		if ($perm_type == 2)
			$result = true;
		elseif ($perm_type == 0 && count(array_intersect($user_info['groups'], $groups)) > 0)
			$result = true;
		elseif ($perm_type == 1 && count(array_intersect($user_info['groups'], $groups)) >= count($groups))
			$result = true;
...
function sp_query_string($tourniquet)
{
	...
	$fix = str_replace('{version}', $sportal_version, '<a href="http://www.simpleportal.net/" target="_blank" class="new_win">SimplePortal {version} &copy; 2008-2010, SimplePortal</a>');
...
function sportal_get_pages($page_id = null, $active = false)
{
	global $smcFunc;
	...
	$request = $smcFunc['db_query']('','
		SELECT
			id_page, namespace, title, body, type, views,
			allowed_groups, permission_type, style, status
...
function sportal_get_shoutbox($shoutbox_id = null, $active = false)
{
	global $smcFunc;
	...
	$request = $smcFunc['db_query']('','
		SELECT
			id_shoutbox, name, allowed_groups, permission_type, warning,
	...
	while ($row = $smcFunc['db_fetch_assoc']($request))
	{
		if (!sp_allowed_to('shoutbox', $row['id_shoutbox'], $row['allowed_groups'], $row['permission_type']))
			continue;
...
{
	global $smcFunc, $scripturl, $context, $user_info, $txt;
	...
	$can_delete = allowedTo(array('sp_admin', 'sp_manage_shoutbox'));
	...
	foreach ($shouts as $shout)
	{
		...
		$shouts[$shout['id']]['text'] = preg_replace('~:jade:~', '<img src="http://www.simpleportal.net/sp/cheerleader.gif" alt="Jade!" />', $shouts[$shout['id']]['text']);
